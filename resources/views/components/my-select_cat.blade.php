...
@endif
 ...
 <div class="">
        <select x-cloak id="select_cat" {{$attributes->merge(['class' => 'w-full border border-gray-200 focus:outline-none focus:ring-0 focus:border-gray-400'])}}>
            @if($hasNullOption)
        ...
        <div class="text-xs text-red-500">{{$message}}</div> @enderror
    </div>

<div x-data="dropdown()" x-init="loadOptions()" class="flex flex-col  mx-auto">
...
function dropdown() {
                return {
                    ...
                    loadOptions() {
                        const options = document.getElementById('select_cat').options;
